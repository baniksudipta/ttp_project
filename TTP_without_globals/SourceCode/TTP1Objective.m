function [fp, ft, ob, Wend]=TTP1Objective(tour,z,items_weight,items_cost,city_dist,vmax,vmin,rent,capacity)
% inputs:
...
% City numbers start from 1
    ...
    ft=0;
    fp=0;
    for i=1:length(tour)-1 %__________O(len(tour))
        h=i+1;
        selecteditem = find(z==tour(i));%_______O(len(items))
        if(~isempty(selecteditem))   %_______Sum over all cities = O(len(items))
            Wc=Wc+sum(items_weight(selecteditem));
            fp=fp+sum(items_cost(selecteditem));
        end;
        ft=ft+(city_dist(tour(i),tour(h))/(vmax-Wc*(vmax-vmin)/capacity));%____O(1)
    end;
    % Overall Complexity O(len(tour)*len(items))
    Wend = capacity - Wc;
    ob=fp-ft*rent;
